...
function M.on_check(name, done)
  ...
  if installation_state[name] or pm.is_plugin_installing(name) then
    vim.notify(string.format("[nix-plugin-manager] Plugin %s is already being installed", name), vim.log.levels.WARN)
    vim.schedule(done)
    return
  end
...
function M.on_uncheck(name, done)
  ...
  if installation_state[name] or pm.is_plugin_installing(name) then
    vim.notify(string.format("[nix-plugin-manager] Cannot remove %s: installation in progress", name), vim.log.levels.WARN)
    vim.schedule(done)
    return
  end
