local window = require("nix.config").options.window
...
local DEFAULT_CONFIG = {
	...
	icons = {
		checked = window.icons.checked,
		unchecked = window.icons.unchecked,
		pending_frames = window.icons.pending_frames,
		pending = window.icons.pending,
	},
	spinner_interval = 80, -- ms
	keys = {
		check = window.keys.check,
		uncheck = window.keys.uncheck,
		toggle = window.keys.toggle,
		sort = window.keys.sort,
		help = window.keys.help,
		refresh = window.keys.refresh,
		close = window.keys.close,
	},
	highlights = {
...
function Adapter:item_at_cursor()
	...
	-- Calculate the offset based on the maximum length of configured icons
	local checked_len = #self.config.icons.checked
	local unchecked_len = #self.config.icons.unchecked
	local max_icon_len = math.max(checked_len, unchecked_len)
	-- Account for the leading space and trailing space after icon: " [icon] item"
	local offset = 1 + max_icon_len + 1
	local item = line:sub(offset + 1) -- +1 to skip the space after icon
