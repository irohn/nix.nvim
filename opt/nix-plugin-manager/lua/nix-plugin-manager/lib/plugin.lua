...
function Plugin:install(opts, on_complete)
	...
	local build_opts = vim.tbl_deep_extend("force", opts, {
		...
		on_exit = function(obj)
			...
			if success then
				...
				vim.schedule(function()
					vim.notify(
						string.format("Failed to install plugin %s: %s", self.name, obj.stderr or "Unknown error"),
	...
	timeout_timer:start(timeout_ms, 0, function()
		...
		vim.schedule(function()
			vim.notify(
				string.format("Plugin %s installation timed out after 5 minutes", self.name),
				vim.log.levels.ERROR
			)
			on_complete(false)
		end)
...
Plugin.__tostring = function(self)
    ...
    NixPlugin(
      ...
      config=%s,
		...
		self.name,
		self.pname,
		tostring(self.package),
		vim.inspect(self.dependencies),
		tostring(self.init),
