...
function M.install_and_load_plugins()
	...
	local function install_next()
		...
		save_installation_state()
		vim.notify(
			string.format("[nix-plugin-manager] Installing plugin %s (%d/%d)", name, install_index, #plugins_to_install),
			vim.log.levels.INFO
		)
...
function M.load_configured_plugins()
	...
	local function load_plugin_with_deps(name)
		...
		if state.installing_plugins[name] then
			vim.notify(
				string.format("[nix-plugin-manager] Skipping %s: installation in progress", name),
				vim.log.levels.INFO
			)
			return false
		end
			...
			for _, dep_name in ipairs(state.dependency_graph[name]) do
				...
				if dep_plugin then
					if not load_plugin_with_deps(dep_name) then
						vim.notify(
							string.format("[nix-plugin-manager] Failed to load dependency %s for %s", dep_name, name),
					...
					vim.notify(
						string.format("[nix-plugin-manager] Dependency %s for %s is not configured", dep_name, name),
						vim.log.levels.WARN
					)
				end
			end
...
function M.install_plugin(name, on_complete)
	...
	plugin:install(function(success)
		...
		if success then
			...
			vim.notify(
				string.format("[nix-plugin-manager] Plugin %s installed successfully", name),
				vim.log.levels.INFO
			)
		else
