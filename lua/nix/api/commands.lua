...
function M.build(package, data_dir, flakes, nixpkgs_url)
	...
	vim.schedule(function()
		notify(string.format("Building %s ...", package))
	end)

	vim.system(cmd, { text = true }, function(obj)
		if obj.code == 0 then
			vim.schedule(function()
				notify(string.format("Successfully built %s at %s", package, outlink))
			end)
		else
			vim.schedule(function()
				notify(
					string.format("Failed to build package %s:\n%s", package, obj.stderr or ""),
		...
		if obj.code == 0 then
			vim.schedule(function()
				...
				if callback then
					...
					table.insert(
						lines,
						string.format("Search results for '%s' (%d packages found):", search_string, #packages)
					)
